...
def gift2(GiftObjectList,CoupleObjectList):
	for cp in CoupleObjectList:
		i=0
		j=0
		k=0
		for gft in GiftObjectList:
			if gft.type ==  "Essential" and i==0:
				cp.gifts.append(gft)
				i=i+1
			elif gft.type == "Luxury" and j== 0 :
				cp.gifts.append(gft) 
				j = j+1
			elif gft.type == "utility" and k == 0:
				cp.gifts.append(gft) 
				k = k+1
		cp.boy.budget = int(cp.boy.budget) - int(int(cp.gifts[0].cost)+int(cp.gifts[1].cost)+int(cp.gifts[2].cost))
